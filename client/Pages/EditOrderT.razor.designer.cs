...
{
    public partial class EditOrderTComponent : ComponentBase
    {
        ...
        TiomsOrders.Models.To.Customer _getByCustomersForCustomerIDResult;
        protected TiomsOrders.Models.To.Customer getByCustomersForCustomerIDResult
        {
            get
            {
                return _getByCustomersForCustomerIDResult;
            }
            set
            {
                if (!object.Equals(_getByCustomersForCustomerIDResult, value))
                {
                    var args = new PropertyChangedEventArgs(){ Name = "getByCustomersForCustomerIDResult", NewValue = value, OldValue = _getByCustomersForCustomerIDResult };
                    _getByCustomersForCustomerIDResult = value;
                    OnPropertyChanged(args);
                    Reload();
                }
            }
        ...
        TiomsOrders.Models.To.Product _getByProductsForProductIDResult;
        protected TiomsOrders.Models.To.Product getByProductsForProductIDResult
        {
            get
            {
        ...
        {
            var toGetOrderTByOrderIdResult = await To.GetOrderTByOrderId(orderId:OrderID);
            ordert = toGetOrderTByOrderIdResult;

            if (this.ordert.CustomerID != null)
            {
                var toGetCustomerByCustomerIdResult = await To.GetCustomerByCustomerId(customerId:this.ordert.CustomerID);
                getByCustomersForCustomerIDResult = toGetCustomerByCustomerIdResult;
            }

            if (this.ordert.ProductID != null)
            {
                var toGetProductByProductIdResult = await To.GetProductByProductId(productId:this.ordert.ProductID);
                getByProductsForProductIDResult = toGetProductByProductIdResult;
            }
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(TiomsOrders.Models.To.OrderT args)
        {
            try
            {
                var toUpdateOrderTResult = await To.UpdateOrderT(orderId:OrderID, orderT:ordert);
                DialogService.Close(ordert);
            }
            catch (System.Exception toUpdateOrderTException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to update OrderT" });
            }
        }
